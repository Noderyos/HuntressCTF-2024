...
def read_section(f):
    ...
    if chk_type[:3] == b"biT":
        ...
        for i in range(len(chk_data)):
            data.append(chk_data[i]^chk_type[i%4])
        flag_dict[chk_type] = bytes(data)
    return 4 + 4 + length + 4
...
print("Extracted section : ", flag_dict)

for k in sorted(flag_dict):
    for c in flag_dict[k]:
        if c < 127: print(chr(c), end="")
print()
